import type { BracketOutput, ResolvedMatch, Round } from '@/lib/tournament/bracket';
...
import { getSlotMatchNumber } from '@/lib/tournament/slotLabels';
...
const BRACKET_CARD_HEIGHT_REM = 8.55;
const FIRST_ROUND_GAP_REM = 2.3;
...
export default function ParticipantPredictionBracketView({
  ...
  const renderPositionedRound = (
    ...
    return (
      <section key={`${side}-${round}`} className="w-60 shrink-0 space-y-5">
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        aria-label="Llave de eliminatorias predicha con desplazamiento horizontal"
      >
        <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-zinc-800 dark:bg-zinc-950/40">
          <div className="flex min-w-max items-start gap-8">
            ...
            <section className="w-64 shrink-0 space-y-5">
